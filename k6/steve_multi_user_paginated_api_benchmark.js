...
const usernamePrefix = __ENV.USERNAME_PREFIX
const numUsers = __ENV.NUMUSERS || 5
const password = __ENV.PASSWORD
const token = __ENV.TOKEN
const cluster = __ENV.CLUSTER || "local"
const resource = __ENV.RESOURCE || "management.cattle.io.namespaces"
...
export function setup() {
    ...
    let usersCookies = {}
    const usernames = Array(numUsers).fill(null).map((_, index) => {
    ...
    usernames.forEach((username) => {
        ...
        usersCookies[username] = http.cookieJar().cookiesForURL(res.url)
    })

    return usersCookies
}

export function list(usersCookies) {
    // select random user. random() uses a uniform distribution by default
    const randomUsername = usernamePrefix + Math.floor(1 + (Math.random() * numUsers))
    const cookies = usersCookies[randomUsername]
